...
x = np.array([1.3, -2.2, 9.5])
w = np.array([-0.76, 0.22, 0.56])
...
for item in x_test:
    h1_in = np.dot(item, w0) + b1 # this calculates the linear combination of inputs and weights.
    h1_out = hidden_activation(h1_in) # apply activation function
    
    h2_in = np.dot(h1_out, w1) + b2 # the output of the previous layer is the input for this layer. it is missing the bias term, fix it!
